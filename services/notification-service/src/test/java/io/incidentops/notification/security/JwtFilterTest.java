package io.incidentops.notification.security;
...
class JwtFilterTest {
    ...
    @Test
    void notificationApiPathsAreNotExempted() throws Exception {
        var filter = new JwtFilter(new JwtUtil(SECRET));

        assertThat(shouldNotFilter(filter, new MockHttpServletRequest("GET", "/api/notifications"))).isFalse();
    }
}
