package io.incidentops.incident.security;
...
class JwtFilterTest {
    ...
    @Test
    void incidentApiPathsAreNotExempted() throws Exception {
        var filter = new JwtFilter(new JwtUtil(SECRET));

        assertThat(shouldNotFilter(filter, new MockHttpServletRequest("GET", "/api/incidents"))).isFalse();
    }
}
