package io.incidentops.analytics.exception;
...
class GlobalExceptionHandlerTest {
    ...
    private WebRequest webRequest() {
        ...
        when(request.getDescription(false)).thenReturn("uri=/api/analytics/summary");
        return request;
    }

    @Test
    void handleApiDelegatesToBaseExceptionHandlersMapping() {
        var ex = new ApiException(HttpStatus.NOT_FOUND, "incident fact not found", "FACT_NOT_FOUND");

        var response = handler.handleApi(ex, webRequest());

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().code()).isEqualTo("FACT_NOT_FOUND");
        assertThat(response.getBody().message()).isEqualTo("incident fact not found");
    }
