...
class JwtFilterTest {
    ...
    @Test
    void isAJwtAuthFilterConfiguredWithThePublicOperationalPathsPlusWorkflowStates() throws Exception {
        ...
        assertThat(shouldNotFilter(filter, new MockHttpServletRequest("GET", "/api/workflow/states"))).isTrue();
    }

    @Test
    void incidentScopedRoutesAreNotExempted() throws Exception {
        var filter = new JwtFilter(new JwtUtil(SECRET));

        assertThat(shouldNotFilter(filter, new MockHttpServletRequest("GET", "/api/workflow/incidents/inc-1/state"))).isFalse();
        assertThat(shouldNotFilter(filter, new MockHttpServletRequest("POST", "/api/workflow/incidents/inc-1/transition"))).isFalse();
    }
}
